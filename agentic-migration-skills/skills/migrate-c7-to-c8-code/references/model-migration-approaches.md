...
| `camunda:delegateExpression` or `camunda:expression` with a method invocation (expression method) | Apply the expression rule. Remove the method argument list after joining the path segments. | `${sampleBean.someMethod(x)}` becomes `sampleBeanSomeMethod`. |
